...
public class JwtServiceImpl implements JwtService {
  ...
  @Override
  public String createJwsToken(CreateJwsTokenReq req) throws Exception {
    return Jwts.builder()
        .subject(req.getId())
        .claim("email", req.getEmail())
        .claim("role", req.getRole())
        .issuedAt(new Date())
        .expiration(new Date(System.currentTimeMillis() + jwsExpiration))
        .signWith(keyService.getJwsPrivateKey(), signatureAlgorithm)
        .compact();
  }
